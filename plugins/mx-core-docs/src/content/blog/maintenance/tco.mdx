...
[**Kembali ke Atas**](#i-executive-summary)
...
```mermaid
flowchart TB
    A["1. Perencanaan & Perancangan"] --> B["2. Pengadaan"]
    B --> C["3. Penerimaan & Instalasi"]
    C --> D["4. Commissioning & Start-up"]
    D --> E["5. Operasi"]
    E --> F["6. Pemeliharaan & Perbaikan"]
    F --> G["7. Evaluasi Penggantian / Modernisasi"]
    G --> H["8. Decommissioning & Disposal"]
    H --> A

    classDef plan fill:#DDEBFF,stroke:#1E5AA8,color:#0B2545
    classDef procure fill:#E8F5E9,stroke:#2E7D32,color:#123D18
    classDef install fill:#FFF3E0,stroke:#EF6C00,color:#4A2500
    classDef operate fill:#E0F7FA,stroke:#00838F,color:#00363A
    classDef maintain fill:#FCE4EC,stroke:#AD1457,color:#4A1028
    classDef replace fill:#F3E5F5,stroke:#6A1B9A,color:#2A073D
    classDef dispose fill:#ECEFF1,stroke:#455A64,color:#1C2529
...
![tco](/static/images/artikel/tco.jpg)
...
flowchart TB
    ...
    B --> F["Evaluasi Keandalan Aset"]
    C --> F
    D --> F
    E --> F

    F --> G{"Risiko Meningkat?"}

    G -->|Ya| H["Analisis Downtime"]
    G -->|Ya| I["Analisis HSSE / LOC"]
    G -->|Ya| J["Analisis TCO"]
    G -->|Tidak| K["Lanjutkan Strategi PM/PdM"]

    H --> L["Prioritas Intervensi"]
    I --> L
    J --> L

    L --> M["Repair / Retrofit / Replace"]

    classDef data fill:#E3F2FD,stroke:#1565C0,color:#0D2F57
    classDef metric fill:#E8F5E9,stroke:#2E7D32,color:#123D18
    classDef risk fill:#FFEBEE,stroke:#C62828,color:#5A0B0B
    classDef action fill:#FFF8E1,stroke:#F9A825,color:#4A3700
    classDef decision fill:#F3E5F5,stroke:#7B1FA2,color:#2D0A3D
...
![equipment-life-cycle](/static/images/artikel/equipment-life-cycle.png)
...
![tco-intro](/static/images/artikel/tco-intro.jpg)
...
```mermaid
flowchart TB
    A["Mulai: Evaluasi Performa Aset"] --> B{"MTBF turun / OEE rendah?"}

    B -->|Tidak| C["Lanjutkan Operasi & PM"]
    B -->|Ya| D{"Biaya Maintenance meningkat?"}

    D -->|Tidak| E["Optimasi PM / PdM"]
    D -->|Ya| F{"Risiko HSSE / Downtime tinggi?"}

    F -->|Tidak| G["Evaluasi Retrofit / Upgrade"]
    F -->|Ya| H{"TCO aset lama > aset baru?"}

    H -->|Tidak| G
    H -->|Ya| I["Ajukan Replacement CAPEX"]

    G --> J{"Retrofit feasible?"}
    J -->|Ya| K["Laksanakan Modernisasi"]
    J -->|Tidak| I

    I --> L["Penggantian Aset"]
    K --> M["Update Strategi Lifecycle"]
    L --> M

    classDef start fill:#DDEBFF,stroke:#1E5AA8,color:#0B2545
    classDef decision fill:#FFF3E0,stroke:#EF6C00,color:#4A2500
    classDef safe fill:#E8F5E9,stroke:#2E7D32,color:#123D18
    classDef warning fill:#FFEBEE,stroke:#C62828,color:#5A0B0B
    classDef action fill:#F3E5F5,stroke:#6A1B9A,color:#2A073D
...
```mermaid
flowchart TB
    A["Perencanaan & Perancangan<br/>A: Engineering<br/>C: Maintenance, HSSE, Finance"] --> B["Pengadaan<br/>A: Procurement<br/>C: Engineering, Maintenance"]
    B --> C["Penerimaan & Instalasi<br/>A: Engineering<br/>R: Maintenance"]
    C --> D["Commissioning & Start-up<br/>A: Engineering<br/>R: Maintenance, HSSE"]
    D --> E["Operasi<br/>A: Operations / Maintenance<br/>C: Engineering, HSSE"]
    E --> F["Pemeliharaan & Perbaikan<br/>A/R: Maintenance<br/>C: Engineering, HSSE"]
    F --> G["Evaluasi Penggantian<br/>A: Maintenance / Finance<br/>C: Engineering, Procurement"]
    G --> H["Decommissioning & Disposal<br/>A: HSSE / Engineering<br/>R: Maintenance"]

    classDef eng fill:#E3F2FD,stroke:#1565C0,color:#0D2F57
    classDef proc fill:#E8F5E9,stroke:#2E7D32,color:#123D18
    classDef maint fill:#FCE4EC,stroke:#AD1457,color:#4A1028
    classDef hsse fill:#FFEBEE,stroke:#C62828,color:#5A0B0B
    classDef fin fill:#FFF8E1,stroke:#F9A825,color:#4A3700
