...
- [**_Instruksi Kerja (IK/WI) Berbasis Risiko dalam Ekosistem Risk-Based Maintenance (RBM)_**](#instruksi-kerja-ikwi-berbasis-risiko-dalam-ekosistem-risk-based-maintenance-rbm)
...
```mermaid id="ik-wi-rbm-execution-layer-mobile"
flowchart TD
    A["RBM<br/>Risk Decision"] --> B["Job Description<br/>Risk Authority"]
    B --> C["IK/WI<br/>Risk Execution Layer"]
    C --> D["Field Work<br/>Execution"]
    D --> E{"Within<br/>Risk Boundary?"}

    E -- "Yes" --> F["Continue<br/>Controlled Work"]
    E -- "No" --> G["Stop-Work<br/>Escalate"]

    G --> H["Risk Owner<br/>Decision"]
    H --> I["Document<br/>and Feedback"]

    classDef rbm fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef job fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef wi fill:#FFF8E1,stroke:#F9A825,color:#111;
...
```mermaid id="ik-wi-conventional-to-rbm-mobile"
flowchart TD
    A["IK/WI<br/>Konvensional"] --> B["Procedure-Driven<br/>Execution"]
    B --> C["Sequence<br/>Compliance"]
    C --> D["Risk Context<br/>Missing"]
    D --> E["Deviation<br/>Normalized"]

    F["IK/WI<br/>RBM-Compliant"] --> G["Risk-Driven<br/>Execution"]
    G --> H["Risk Trigger<br/>Defined"]
    H --> I["Decision Boundary<br/>Locked"]
    I --> J["Escalation + Stop-Work<br/>Mandatory"]

    classDef conventional fill:#FFEBEE,stroke:#C62828,color:#111;
    classDef rbm fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef middle fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef output fill:#F3E5F5,stroke:#6A1B9A,color:#111;

    class A,B,C,D,E conventional;
    class F,G,H,I,J rbm;
```
...
```mermaid id="rbm-jobdesc-ikwi-hierarchy-mobile"
flowchart TD
    A["MSH-3<br/>RBM"] --> B["Defines<br/>Risk Decision"]
    B --> C["Job Description<br/>RBM-Based"]
    C --> D["Defines<br/>Risk Owner<br/>Authority"]
    D --> E["IK/WI<br/>RBM-Compliant"]
    E --> F["Defines<br/>Execution Boundary"]
    F --> G["Field Work<br/>Controlled by Risk"]
...
```mermaid id="ikwi-risk-assumption-lock-mobile"
flowchart TD
    A["RBM<br/>Risk Assumption"] --> B["Asset Condition"]
    A --> C["Work Method"]
    A --> D["Competent Person"]
    A --> E["Safe Operating<br/>Environment"]

    B --> F["IK/WI<br/>Locks Assumption"]
    C --> F
    D --> F
    E --> F

    F --> G{"Assumption<br/>Still Valid?"}
    G -- "Valid" --> H["Continue<br/>Work"]
    G -- "Invalid" --> I["Stop<br/>Escalate<br/>Reassess"]

    classDef rbm fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef assumption fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef wi fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef decision fill:#F3E5F5,stroke:#6A1B9A,color:#111;
    classDef stop fill:#FFEBEE,stroke:#C62828,color:#111;

    class A rbm;
    class B,C,D,E assumption;
    class F wi;
    class G decision;
    class H assumption;
    class I stop;
...
```mermaid id="mandatory-elements-ikwi-rbm-mobile"
flowchart TD
    A["IK/WI<br/>RBM-Compliant"] --> B["Risk<br/>Context"]
    A --> C["Decision<br/>Boundary"]
    A --> D["Escalation<br/>Obligation"]
    A --> E["Stop-Work<br/>Rule"]

    B --> F["Why Work<br/>is Risk-Sensitive"]
    C --> G["What Field Team<br/>Can / Cannot Decide"]
    D --> H["When Escalation<br/>is Mandatory"]
    E --> I["When Work<br/>Must Stop"]

    F --> J["Risk Enforcement<br/>Tool"]
    G --> J
    H --> J
    I --> J

    classDef wi fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef element fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef function fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef output fill:#F3E5F5,stroke:#6A1B9A,color:#111;

    class A wi;
    class B,C,D,E element;
    class F,G,H,I function;
    class J output;
```
...
```mermaid id="ikwi-rbm-template-structure-mobile"
flowchart TD
    A["IK/WI<br/>RBM Template"] --> B["Identity &<br/>Risk Traceability"]
    B --> C["Risk-Based<br/>Scope"]
    C --> D["RBM & ESC<br/>Context"]
    D --> E["Personnel<br/>Requirement"]
    E --> F["Tools & Integrity<br/>Requirement"]
    F --> G["SHE & Process<br/>Safety Constraint"]
    G --> H["Execution Sequence<br/>with Risk Gate"]
    H --> I["Risk Trigger<br/>Stop-Work"]
    I --> J["Escalation<br/>Authority Boundary"]
    J --> K["Quality Check<br/>Acceptance"]
    K --> L["Documentation<br/>RBM Feedback"]

    classDef template fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef risk fill:#FFEBEE,stroke:#C62828,color:#111;
    classDef control fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef execution fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef feedback fill:#F3E5F5,stroke:#6A1B9A,color:#111;

    class A,B,C,D template;
    class G,I,J risk;
    class E,F,H,K execution;
    class L feedback;
```
...
```mermaid id="work-vs-decision-boundary-mobile"
flowchart TD
    A["Field<br/>Condition"] --> B{"Within<br/>Normal Parameter?"}

    B -- "Yes" --> C["Technician<br/>May Execute Work"]
    C --> D["Work Continues<br/>Within IK/WI"]

    B -- "No" --> E["Decision Required<br/>Beyond Technician Authority"]
    E --> F["Stop Work"]
    F --> G["Escalate to<br/>Risk Owner"]
    G --> H["Supervisor / Engineer<br/>Decision"]
    H --> I["Resume / Change Method<br/>or Hold Work"]

    classDef condition fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef execute fill:#E8F5E9,stroke:#2E7D32,color:#111;
...
```mermaid id="risk-trigger-stopwork-escalation-mobile"
flowchart TD
    A["Work Execution<br/>in Field"] --> B{"Risk Trigger<br/>Appears?"}

    B -- "No" --> C["Continue<br/>Within IK/WI"]
    B -- "Yes" --> D["Mandatory<br/>Stop-Work"]

    D --> E["Secure<br/>Equipment / Area"]
    E --> F["Escalate<br/>Foreman → Supervisor → Engineer"]
    F --> G["Risk Owner<br/>Decision"]
    G --> H{"Decision<br/>Closed?"}

    H -- "No" --> I["Work Remains<br/>Stopped"]
    H -- "Yes" --> J["Resume / Revise<br/>Work Method"]

    classDef work fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef normal fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef trigger fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef stop fill:#FFEBEE,stroke:#C62828,color:#111;
...
```mermaid id="ikwi-by-asset-criticality-mobile"
flowchart TD
    A["Asset<br/>Criticality"] --> B{"Risk<br/>Class?"}

    B -- "Critical Asset" --> C["Hard Barrier<br/>Strict Risk Trigger"]
    B -- "Semi-Critical" --> D["Controlled Flexibility<br/>Supervisor Boundary"]
    B -- "Non-Critical" --> E["Simplified IK/WI<br/>Wider Execution Flexibility"]

    C --> F["Engineer / Superintendent<br/>Escalation Mandatory"]
    D --> G["Documented<br/>Escalation if Limit Exceeded"]
    E --> H["Stop-Work Still Applies<br/>if SHE Risk Appears"]

    F --> I["Control Level<br/>Proportional to Risk"]
    G --> I
    H --> I
    ...
    classDef semi fill:#FFF3E0,stroke:#EF6C00,color:#111;
    classDef non fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef output fill:#F3E5F5,stroke:#6A1B9A,color:#111;
...
```mermaid id="uniform-ikwi-failure-risk-mobile"
flowchart TD
    A["Uniform<br/>IK/WI"] --> B["Same Control<br/>for Different Risk"]
    B --> C{"Asset<br/>Risk Level"}

    C -- "Critical" --> D["Under-Control<br/>High Consequence Exposure"]
    C -- "Non-Critical" --> E["Over-Control<br/>Resource Waste"]

    D --> F["Silent<br/>Risk Drift"]
    E --> G["Low Value<br/>Workload"]

    F --> H["RBM Fails<br/>at Execution Level"]
    G --> H

    classDef uniform fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef decision fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef bad fill:#FFEBEE,stroke:#C62828,color:#111;
    classDef waste fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef output fill:#F3E5F5,stroke:#6A1B9A,color:#111;
...
```mermaid id="ikwi-evaluation-loop-rbm-mobile"
flowchart TD
    A["IK/WI<br/>Field Execution"] --> B["Capture<br/>Near-Miss"]
    A --> C["Capture<br/>Execution Deviation"]
    A --> D["Capture<br/>Decision Delay"]

    B --> E["RBM<br/>Evaluation Loop"]
    C --> E
    D --> E

    E --> F["Update<br/>Risk Register"]
    E --> G["Review<br/>JobDesc Authority"]
    E --> H["Identify<br/>Training Need"]
    E --> I["Revise<br/>IK/WI"]

    F --> J["Living Control<br/>Document"]
    G --> J
    H --> J
    I --> J
    ...
    classDef loop fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef update fill:#F3E5F5,stroke:#6A1B9A,color:#111;

    class A,J wi;
    class B,C,D data;
    class E loop;
    class F,G,H,I update;
```
...
```mermaid id="common-ikwi-rbm-errors-mobile"
flowchart TD
    A["Common IK/WI<br/>RBM Errors"] --> B["Over-Technical<br/>No Risk Context"]
    A --> C["Copy-Paste<br/>Across Assets"]
    A --> D["No Escalation<br/>Clause"]
    A --> E["Acceptance<br/>Without Authority"]
    A --> F["Depends on<br/>Personal Experience"]

    B --> G["RBM Fails<br/>in Execution"]
    C --> G
    D --> G
    E --> G
    F --> G

    G --> H["No Clear<br/>Risk Control"]
    H --> I["Repeat Deviation<br/>or Incident"]
...
```mermaid id="anti-overlap-ikwi-scope-mobile"
flowchart TD
    A["Artikel Ini<br/>Fokus"] --> B["IK/WI<br/>RBM-Compliant"]
    B --> C["Risk Context"]
    B --> D["Decision Boundary"]
    B --> E["Escalation Logic"]
    B --> F["Stop-Work Rule"]
    B --> G["Execution Feedback"]

    A --> H["Tidak Dibahas"]
    H --> I["Filosofi RBM"]
    H --> J["Desain Organisasi"]
    H --> K["Strategi Manpower"]
    H --> L["Detail JobDesc"]
    H --> M["Training Teknis<br/>per Disiplin"]
...
```mermaid id="ikwi-final-risk-control-summary-mobile"
flowchart TD
    A["RBM<br/>Risk Register / Matrix"] --> B["Risk-Based<br/>Job Description"]
    B --> C["IK/WI<br/>RBM-Compliant"]
    C --> D["Field<br/>Execution"]

    D --> E["Work Sequence<br/>Controlled"]
    D --> F["Decision Boundary<br/>Respected"]
    D --> G["Stop-Work<br/>Activated"]
    D --> H["Escalation<br/>Documented"]

    E --> I["Risk Control<br/>Becomes Real"]
    F --> I
    G --> I
    H --> I
...
```mermaid id="rotating-equipment-ikwi-control-flow-mobile"
flowchart TD
    A["Rotating Equipment<br/>IK/WI"] --> B["Isolation<br/>Zero Energy"]
    B --> C["Initial Check<br/>Alignment / Condition"]
    C --> D["Disassembly<br/>Bearing / Seal / Coupling"]
    D --> E{"Abnormality<br/>Found?"}

    E -- "No" --> F["Reassembly<br/>Torque / Fit-Up"]
    E -- "Yes" --> G["Stop-Work<br/>Escalate Engineer"]

    F --> H["Final Alignment<br/>Within Tolerance"]
    H --> I["Test Run<br/>Vibration / Temp"]
    I --> J{"Acceptance<br/>OK?"}

    J -- "Yes" --> K["Document<br/>RBM Feedback"]
    J -- "No" --> G

    classDef rotating fill:#E3F2FD,stroke:#1565C0,color:#111;
...
```mermaid id="rotating-risk-trigger-stopwork-mobile"
flowchart TD
    A["Rotating Work<br/>In Progress"] --> B{"Risk Trigger?"}

    B -- "Vibration > Alarm" --> C["Stop"]
    B -- "Shaft / Bearing<br/>Scoring" --> C
    B -- "Misalignment<br/>Out of Tolerance" --> C
    B -- "Oil Contamination<br/>Severe" --> C
    B -- "Critical Bolt<br/>Torque Failed" --> C

    C --> D["Secure<br/>Equipment"]
    D --> E["Escalate to<br/>Supervisor / Engineer"]
    E --> F["Decision:<br/>Repair / Replace / Rework"]
    F --> G["Document<br/>Finding"]
...
```mermaid id="instrument-control-ikwi-barrier-flow-mobile"
flowchart TD
    A["Instrument & Control<br/>IK/WI"] --> B["Verify<br/>Tag / Loop / Logic"]
    B --> C["Isolate<br/>Process + Logic"]
    C --> D["Calibration<br/>Check"]
    D --> E["Loop<br/>Checking"]
    E --> F["Functional<br/>Test"]
    F --> G{"Logic / Trip<br/>Response OK?"}

    G -- "Yes" --> H["Restore<br/>Bypass / Normal Logic"]
    G -- "No" --> I["Stop-Work<br/>Escalate Engineer"]

    H --> J["Acceptance<br/>Engineer / Superintendent"]
    J --> K["Document<br/>Calibration + Bypass Log"]

    classDef inst fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef work fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef decision fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef stop fill:#FFEBEE,stroke:#C62828,color:#111;
    classDef doc fill:#F3E5F5,stroke:#6A1B9A,color:#111;
...
```mermaid id="instrument-risk-trigger-stopwork-mobile"
flowchart TD
    A["Instrument Work<br/>In Progress"] --> B{"Risk Trigger?"}

    B -- "Calibration<br/>Out of Tolerance" --> C["Stop"]
    B -- "Cause-Effect<br/>Mismatch" --> C
    B -- "Alarm / Trip<br/>Not Appearing" --> C
    B -- "Instrument<br/>Not Repeatable" --> C

    C --> D["Secure<br/>Logic / Process"]
    D --> E["Escalate to<br/>Instrument Engineer"]
    E --> F["Decision:<br/>Recalibrate / Replace / Hold"]
    F --> G["Update<br/>Functional Test Report"]
...
```mermaid id="ikwi-template-discipline-summary-mobile"
flowchart TD
    A["IK/WI<br/>RBM-Compliant"] --> B["Rotating<br/>Equipment"]
    A --> C["Instrument &<br/>Control"]

    B --> D["Locks<br/>Mechanical Integrity"]
    B --> E["Controls<br/>Vibration / Alignment / Seal Risk"]

    C --> F["Locks<br/>Functional Barrier"]
    C --> G["Controls<br/>Trip / Alarm / Logic Risk"]

    D --> H["Field Risk<br/>Controlled"]
    E --> H
    F --> H
    G --> H

    H --> I["RBM Executed<br/>at Workface Level"]

    classDef main fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef rotating fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef instrument fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef output fill:#F3E5F5,stroke:#6A1B9A,color:#111;

    class A main;
    class B,D,E rotating;
    class C,F,G instrument;
    class H,I output;
```
