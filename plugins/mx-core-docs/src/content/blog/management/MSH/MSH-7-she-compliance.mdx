...
- [**_SHE sebagai Batas Sistem dalam Pengambilan Keputusan Pemeliharaan_**](#she-sebagai-batas-sistem-dalam-pengambilan-keputusan-pemeliharaan)
- [📌 Prolog Modul](#-prolog-modul)
- [7.1 SHE dalam Sistem Pemeliharaan: Dari Kewajiban ke Batas Keputusan](#71-she-dalam-sistem-pemeliharaan-dari-kewajiban-ke-batas-keputusan)
...
- [7.2 SHE Bukan Checklist, tetapi _Decision Boundary_](#72-she-bukan-checklist-tetapi-decision-boundary)
...
- [7.3 ESC sebagai Integrator Risiko Teknik dan SHE](#73-esc-sebagai-integrator-risiko-teknik-dan-she)
...
- [7.4 SHE sebagai Alasan Sah Mengabaikan Optimasi Biaya](#74-she-sebagai-alasan-sah-mengabaikan-optimasi-biaya)
...
- [7.5 Integrasi SHE dalam RBM dan Strategi Pemeliharaan](#75-integrasi-she-dalam-rbm-dan-strategi-pemeliharaan)
...
- [7.6 Peran Jabatan dalam Menjaga Batas SHE](#76-peran-jabatan-dalam-menjaga-batas-she)
...
- [7.7 SHE dalam Konteks Audit, Regulasi, dan Lisensi Operasi](#77-she-dalam-konteks-audit-regulasi-dan-lisensi-operasi)
...
- [7.8 SHE sebagai Sistem Penjaga Keputusan Maintenance](#78-she-sebagai-sistem-penjaga-keputusan-maintenance)
...
```mermaid id="she-as-hard-boundary-overview-mobile"
flowchart TD
    A["Maintenance<br/>Decision System"] --> B["Risk Logic<br/>Module 2"]
    A --> C["RBM Strategy<br/>Module 3"]
    A --> D["Authority<br/>Module 5"]
    A --> E["Execution Learning<br/>Module 6"]

    B --> F["SHE<br/>Hard Boundary"]
    C --> F
    D --> F
    E --> F

    F --> G{"Decision<br/>Allowed?"}
    G -- "Inside SHE Boundary" --> H["Proceed<br/>with Control"]
    G -- "Violates SHE" --> I["Reject / Stop / Escalate"]

    classDef system fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef module fill:#E8F5E9,stroke:#2E7D32,color:#111;
...
```mermaid id="she-from-compliance-to-decision-boundary-mobile"
flowchart TD
    A["SHE<br/>Conventional View"] --> B["Administrative<br/>Compliance"]
    B --> C["Permit<br/>Checklist<br/>Toolbox"]

    D["SHE<br/>System View"] --> E["Decision<br/>Boundary"]
    E --> F["Worker Safety"]
    E --> G["Health Protection"]
    E --> H["Environment"]
    E --> I["License to Operate"]

    F --> J["Decision<br/>Valid / Invalid"]
    G --> J
    H --> J
    I --> J
    ...
    classDef dimension fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef output fill:#F3E5F5,stroke:#6A1B9A,color:#111;

    class A,B,C conventional;
    class D system;
    class E boundary;
    class F,G,H,I dimension;
    class J output;
```
...
# 7.2 SHE Bukan Checklist, tetapi _Decision Boundary_
...
```mermaid id="she-no-go-line-maintenance-mobile"
flowchart TD
    A["Maintenance<br/>Decision"] --> B{"Touches<br/>SHE Risk?"}

    B -- "No" --> C["Evaluate<br/>Technical + Cost"]
    B -- "Yes" --> D{"Within<br/>SHE Boundary?"}

    D -- "Yes" --> E["Proceed<br/>with Control"]
    D -- "No" --> F["NO-GO<br/>Decision Rejected"]

    F --> G["Stop Work<br/>or Escalate"]
    G --> H["Reassess<br/>Method / Scope / Timing"]

    classDef input fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef decision fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef proceed fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef nogo fill:#FFEBEE,stroke:#C62828,color:#111;
...
```mermaid id="esc-integrator-technical-she-business-mobile"
flowchart TD
    A["Technical<br/>Failure Scenario"] --> B["ESC<br/>Consequence Lens"]

    B --> C["Environmental"]
    B --> D["Safety"]
    B --> E["Continuous<br/>Running"]

    C --> F["Pollution<br/>Regulatory Impact"]
    D --> G["Injury<br/>Fire / Explosion"]
    E --> H["Shutdown<br/>Business Loss"]

    F --> I["Integrated<br/>Risk Decision"]
    G --> I
    H --> I

    I --> J["Engineer + SHE + Management<br/>Common Language"]

    classDef technical fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef esc fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef env fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef safety fill:#FFEBEE,stroke:#C62828,color:#111;
    classDef running fill:#F3E5F5,stroke:#6A1B9A,color:#111;
    classDef output fill:#ECEFF1,stroke:#455A64,color:#111;
...
```mermaid id="she-overrides-cost-optimization-mobile"
flowchart TD
    A["Cost Optimization<br/>Proposal"] --> B{"SHE Impact<br/>Present?"}

    B -- "No Significant Impact" --> C["Optimize<br/>with Control"]
    B -- "Yes" --> D{"Risk<br/>Acceptable?"}

    D -- "Acceptable" --> E["Proceed<br/>with Mitigation"]
    D -- "Not Acceptable" --> F["Reject<br/>Cost Saving"]

    F --> G["Tighten<br/>Maintenance"]
    F --> H["Early<br/>Replacement"]
    F --> I["Increase<br/>Inspection / Protection"]

    G --> J["Defensible<br/>SHE Decision"]
    H --> J
    I --> J

    classDef proposal fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef decision fill:#FFF8E1,stroke:#F9A825,color:#111;
...
```mermaid id="she-integrated-rbm-strategy-mobile"
flowchart TD
    A["RBM<br/>Risk-Based Strategy"] --> B["Assess<br/>Asset Risk"]
    B --> C{"Has SHE<br/>Consequence?"}

    C -- "Yes" --> D["Classify as<br/>Critical Asset"]
    C -- "No" --> E["Classify by<br/>Operational Risk"]

    D --> F["Stricter<br/>Inspection"]
    D --> G["CBM / PdM<br/>Preferred"]
    D --> H["MOC + SHE<br/>Review Required"]

    E --> I["Proportional<br/>Maintenance Strategy"]

    F --> J["RBM Works<br/>Inside SHE Boundary"]
    G --> J
    H --> J
    I --> J
...
```mermaid id="role-layered-defense-she-mobile"
flowchart TD
    A["Manager<br/>Policy + Resource"] --> B["Superintendent<br/>Operational Gatekeeper"]
    B --> C["Supervisor<br/>Direct Control"]
    C --> D["Foreman<br/>Field Discipline"]
    D --> E["Technician<br/>SOP + Hazard Report"]

    F["Engineer / Senior Engineer<br/>Inherently Safe Solution"] --> B
    F --> C

    A --> G["Layered<br/>SHE Defense"]
    B --> G
    C --> G
    D --> G
    E --> G
    ...
    G --> H["SHE Boundary<br/>Maintained"]

    classDef manager fill:#FFEBEE,stroke:#C62828,color:#111;
    classDef superintendent fill:#F3E5F5,stroke:#6A1B9A,color:#111;
    classDef supervisor fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef foreman fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef tech fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef engineer fill:#E0F2F1,stroke:#00796B,color:#111;
    classDef output fill:#ECEFF1,stroke:#455A64,color:#111;
...
```mermaid id="she-audit-regulation-license-mobile"
flowchart TD
    A["Maintenance<br/>Decision"] --> B["SHE<br/>Justification"]
    B --> C["Traceable<br/>Evidence"]
    C --> D["Audit<br/>Readiness"]

    D --> E["Internal<br/>Audit"]
    D --> F["External<br/>Regulator"]
    D --> G["License to<br/>Operate"]

    B --> H{"SHE Basis<br/>Clear?"}
    H -- "Yes" --> I["Decision<br/>Defensible"]
    H -- "No" --> J["High Audit<br/>and Legal Risk"]

    classDef decision fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef she fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef audit fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef good fill:#F3E5F5,stroke:#6A1B9A,color:#111;
...
```mermaid id="she-guardrail-maintenance-system-mobile"
flowchart TD
    A["Maintenance<br/>Decision"] --> B["Technical<br/>Logic"]
    A --> C["Cost<br/>Logic"]
    A --> D["Schedule<br/>Logic"]

    B --> E["SHE<br/>Guardrail"]
    C --> E
    D --> E

    E --> F{"Inside<br/>Guardrail?"}
    F -- "Yes" --> G["Decision<br/>Can Proceed"]
    F -- "No" --> H["Decision<br/>Invalid"]

    H --> I["Stop<br/>Reject<br/>Escalate"]
    G --> J["Execute<br/>and Document"]

    classDef input fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef logic fill:#E8F5E9,stroke:#2E7D32,color:#111;
...
```mermaid id="module-7-she-final-logic-mobile"
flowchart TD
    A["SHE<br/>Hard Boundary"] --> B["Rejects<br/>Unsafe Decision"]
    A --> C["Overrides<br/>Cost Optimization"]
    A --> D["Limits<br/>Risk Acceptance"]
    A --> E["Guides<br/>RBM Strategy"]
    A --> F["Protects<br/>License to Operate"]

    B --> G["Safe<br/>Maintenance System"]
    C --> G
    D --> G
    E --> G
    F --> G

    G --> H["Legal<br/>Ethical<br/>Defensible"]
...
```mermaid id="module-7-connection-to-msh-mobile"
flowchart TD
    A["Module 2<br/>Risk Acceptance"] --> B["Module 7<br/>SHE Hard Boundary"]
    C["Module 3<br/>RBM Strategy"] --> B
    D["Module 5<br/>Authority & Accountability"] --> B
    E["Module 6<br/>Execution & Learning"] --> B

    B --> F["Defines<br/>Where Decision Must Stop"]
    F --> G["Module 8<br/>Lifecycle & Continuous Improvement"]

    classDef previous fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef she fill:#FFEBEE,stroke:#C62828,color:#111;
    classDef output fill:#F3E5F5,stroke:#6A1B9A,color:#111;
    classDef lifecycle fill:#E8F5E9,stroke:#2E7D32,color:#111;

    class A,C,D,E previous;
    class B,F she;
    class G lifecycle;
```
