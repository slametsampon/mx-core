...
- [**_Organisasi, KPI, dan Akuntabilitas dalam Sistem Pemeliharaan_**](#organisasi-kpi-dan-akuntabilitas-dalam-sistem-pemeliharaan)
- [📌 Prolog Modul](#-prolog-modul)
- [5.1 Organisasi Pemeliharaan sebagai Sistem, Bukan Sekadar Struktur](#51-organisasi-pemeliharaan-sebagai-sistem-bukan-sekadar-struktur)
- [5.2 Job Description sebagai Fondasi Akuntabilitas](#52-job-description-sebagai-fondasi-akuntabilitas)
- [5.3 Pembagian Peran dan Tanggung Jawab per Level Jabatan](#53-pembagian-peran-dan-tanggung-jawab-per-level-jabatan)
  - [Struktur Jabatan dan Mandat Utama](#struktur-jabatan-dan-mandat-utama)
- [5.4 KPI Berbasis Peran: Mengukur yang Relevan](#54-kpi-berbasis-peran-mengukur-yang-relevan)
...
- [5.5 KPI sebagai Alat Pengambilan Keputusan, Bukan Sekadar Laporan](#55-kpi-sebagai-alat-pengambilan-keputusan-bukan-sekadar-laporan)
- [5.6 Kepemilikan Anggaran dan Tanggung Jawab Biaya](#56-kepemilikan-anggaran-dan-tanggung-jawab-biaya)
...
- [5.7 Governance, Authority, dan Jalur Keputusan](#57-governance-authority-dan-jalur-keputusan)
...
- [5.8 Organisasi Pemeliharaan sebagai Sistem yang Dapat Diaudit](#58-organisasi-pemeliharaan-sebagai-sistem-yang-dapat-diaudit)
...
```mermaid id="msh-5-organization-accountability-overview-mobile"
flowchart TD
    ...
    D --> E["Job Description<br/>Clear Role"]
    D --> F["KPI<br/>Relevant Measure"]
    D --> G["Authority<br/>Decision Boundary"]
    D --> H["Governance<br/>Auditability"]

    E --> I["Accountable<br/>Maintenance System"]
    F --> I
    G --> I
    H --> I
    ...
    classDef element fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef output fill:#F3E5F5,stroke:#6A1B9A,color:#111;

    class A,B,C foundation;
    class D module;
    class E,F,G,H element;
    class I output;
```
...
```mermaid id="maintenance-organization-as-system-mobile"
flowchart TD
    A["Maintenance<br/>Organization"] --> B["Clear<br/>Role"]
    A --> C["Clear<br/>Decision Flow"]
    A --> D["Clear<br/>Authority"]
    A --> E["Clear<br/>Accountability"]

    B --> F["Fast Technical<br/>Decision"]
    C --> F
    D --> G["Controlled<br/>Execution"]
    E --> H["Traceable<br/>Responsibility"]

    F --> I["Higher<br/>Reliability"]
    G --> I
    H --> I

    I --> J["Lower Downtime<br/>Lower Risk<br/>Better Cost Control"]

    classDef org fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef clarity fill:#E8F5E9,stroke:#2E7D32,color:#111;
...
```mermaid id="jobdesc-kpi-authority-accountability-chain-mobile"
flowchart TD
    A["Job Description<br/>Mandate & Boundary"] --> B["KPI<br/>Relevant to Role"]
    B --> C["Authority<br/>Right to Decide"]
    C --> D["Accountability<br/>Owner of Result"]

    D --> E{"Decision<br/>Traceable?"}
    E -- "Yes" --> F["System<br/>Defensible"]
    E -- "No" --> G["Grey Area<br/>No Clear Owner"]

    classDef job fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef kpi fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef auth fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef acc fill:#F3E5F5,stroke:#6A1B9A,color:#111;
...
```mermaid id="maintenance-role-ownership-ladder-mobile"
flowchart TD
    A["Manager Maintenance<br/>System & Policy Owner"] --> B["Superintendent<br/>Operational Execution Owner"]
    B --> C["Supervisor<br/>Daily Control Owner"]
    C --> D["Foreman<br/>Field Coordination Owner"]
    D --> E["Technician<br/>Execution Quality Owner"]

    F["Engineer / Senior Engineer<br/>Technical Solution Owner"] --> C
    F --> B

    A -. "Policy & Risk Direction" .-> F
    F -. "Technical Recommendation" .-> D
    C -. "Daily Control" .-> E
...
```mermaid id="role-based-kpi-map-mobile"
flowchart TD
    A["Role-Based<br/>Maintenance KPI"] --> B["Manager<br/>System Health"]
    A --> C["Superintendent<br/>Execution Consistency"]
    A --> D["Supervisor<br/>Daily Performance"]
    A --> E["Foreman<br/>Field Effectiveness"]
    A --> F["Technician<br/>Work Quality"]
    A --> G["Engineer<br/>Systemic Problem Solving"]

    B --> B1["Budget Compliance<br/>Availability"]
    C --> C1["Schedule Compliance<br/>CM vs PM Ratio"]
    D --> D1["MTTR / MTBF<br/>WO Accuracy"]
    E --> E1["Job Completion<br/>First-Time Fix"]
    F --> F1["Quality Execution<br/>Standard Compliance"]
    G --> G1["Failure Recurrence<br/>RCA Quality"]

    classDef kpi fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef role fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef metric fill:#FFF8E1,stroke:#F9A825,color:#111;

    class A kpi;
    class B,C,D,E,F,G role;
    class B1,C1,D1,E1,F1,G1 metric;
```
...
```mermaid id="kpi-as-decision-tool-mobile"
flowchart TD
    A["KPI Data<br/>Maintenance Performance"] --> B["Trend<br/>Analysis"]
    B --> C{"Signal<br/>Detected?"}

    C -- "No" --> D["Continue<br/>Monitoring"]
    C -- "Yes" --> E["System<br/>Diagnosis"]

    E --> F["Prioritize<br/>Asset / Area"]
    E --> G["Allocate<br/>Resource"]
    E --> H["Improve<br/>Maintenance Strategy"]

    F --> I["Decision<br/>Action Plan"]
    G --> I
    H --> I

    I --> J["Follow Up<br/>and Review"]
...
```mermaid id="budget-ownership-accountability-mobile"
flowchart TD
    A["Maintenance Budget<br/>as Risk Control Resource"] --> B["Manager<br/>System Budget Owner"]
    A --> C["Superintendent<br/>Operational Budget Owner"]
    A --> D["Supervisor / Foreman<br/>Daily Cost Control"]
    A --> E["Engineer<br/>Technical Cost Justification"]

    B --> F["Risk-Based<br/>Budget Alignment"]
    C --> G["Priority & Resource<br/>Allocation"]
    D --> H["Material / Overtime<br/>Control"]
    E --> I["Repair / Replace<br/>Justification"]

    F --> J["Cost Decision<br/>Traceable to Owner"]
    G --> J
    H --> J
    I --> J

    classDef budget fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef owner fill:#E8F5E9,stroke:#2E7D32,color:#111;
...
```mermaid id="maintenance-governance-decision-path-mobile"
flowchart TD
    A["Maintenance<br/>Issue / Proposal"] --> B{"Decision<br/>Type?"}

    B -- "Technical" --> C["Engineer<br/>Recommendation"]
    B -- "Cost" --> D["Supervisor / Superintendent<br/>Cost Review"]
    B -- "Risk / SHE" --> E["SHE + Maintenance<br/>Risk Review"]

    C --> F{"Impact<br/>High?"}
    D --> F
    E --> F

    F -- "No" --> G["Approved at<br/>Operational Level"]
    F -- "Yes" --> H["Escalate to<br/>Manager Maintenance"]

    H --> I["Policy / Budget / Risk<br/>Approval"]
    G --> J["Execute<br/>and Document"]
    I --> J

    classDef issue fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef decision fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef review fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef escalate fill:#FFEBEE,stroke:#C62828,color:#111;
    classDef output fill:#F3E5F5,stroke:#6A1B9A,color:#111;

    class A issue;
    class B,F decision;
    class C,D,E,G review;
    class H,I escalate;
    class J output;
```
...
```mermaid id="audit-ready-maintenance-organization-mobile"
flowchart TD
    A["Audit-Ready<br/>Maintenance Organization"] --> B["Job Description<br/>Role Clarity"]
    A --> C["KPI<br/>Performance Evidence"]
    A --> D["Governance<br/>Decision Path"]
    A --> E["Documentation<br/>Decision Record"]

    B --> F["Who Decided?"]
    C --> G["Why Action<br/>Was Needed?"]
    D --> H["With What<br/>Authority?"]
    E --> I["Where is<br/>the Evidence?"]

    F --> J["Traceable<br/>Decision Chain"]
    G --> J
    H --> J
    I --> J

    J --> K["Defensible<br/>in Audit"]

    classDef audit fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef element fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef question fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef chain fill:#F3E5F5,stroke:#6A1B9A,color:#111;
    classDef output fill:#FFEBEE,stroke:#C62828,color:#111;

    class A audit;
    class B,C,D,E element;
    class F,G,H,I question;
    class J chain;
    class K output;
```
...
```mermaid id="organization-kpi-accountability-final-summary-mobile"
flowchart TD
    A["Clear<br/>Organization"] --> B["Clear<br/>Job Description"]
    B --> C["Relevant<br/>Role-Based KPI"]
    C --> D["Defined<br/>Authority"]
    D --> E["Clear<br/>Budget Ownership"]
    E --> F["Strong<br/>Governance"]
    F --> G["Audit-Ready<br/>Maintenance System"]

    G --> H["Reliable<br/>Defensible<br/>Sustainable"]

    classDef org fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef job fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef kpi fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef gov fill:#F3E5F5,stroke:#6A1B9A,color:#111;
    classDef output fill:#FFEBEE,stroke:#C62828,color:#111;
