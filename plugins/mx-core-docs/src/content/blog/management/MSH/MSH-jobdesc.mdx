...
- [**_Job Description Berbasis Risiko dalam Ekosistem Risk-Based Maintenance (RBM)_**](#job-description-berbasis-risiko-dalam-ekosistem-risk-based-maintenance-rbm)
...
```mermaid id="jobdesc-rbm-overview-mobile"
flowchart TD
    A["RBM Strategy"] --> B["Asset Risk<br/>Criticality"]
    B --> C["Role & Authority"]
    C --> D["Escalation<br/>Obligation"]
    D --> E["Stop-Work<br/>Right"]
    E --> F["Risk Ownership"]
    F --> G["Auditable<br/>Job Description"]

    classDef strategy fill:#E3F2FD,stroke:#1565C0,color:#0D1B2A;
    classDef risk fill:#FFEBEE,stroke:#C62828,color:#0D1B2A;
    classDef role fill:#E8F5E9,stroke:#2E7D32,color:#0D1B2A;
    classDef action fill:#FFF8E1,stroke:#F9A825,color:#0D1B2A;
    classDef owner fill:#F3E5F5,stroke:#6A1B9A,color:#0D1B2A;
...
```mermaid id="jobdesc-konvensional-vs-rbm-mobile"
flowchart TD
    A["JobDesc<br/>Konvensional"] --> B["Daftar Tugas<br/>Administratif"]
    B --> C["Authority<br/>Tidak Jelas"]
    C --> D["Eskalasi<br/>Informal"]
    D --> E["Stop-Work<br/>Ragu-ragu"]
    E --> F["RBM Menjadi<br/>Paper System"]

    A2["JobDesc<br/>Berbasis RBM"] --> B2["Decision Scope<br/>Jelas"]
    B2 --> C2["Authority Limit<br/>Tegas"]
    C2 --> D2["Mandatory<br/>Escalation"]
    D2 --> E2["Stop-Work<br/>Dilindungi"]
    E2 --> F2["RBM Menjadi<br/>Living System"]

    classDef bad fill:#FFEBEE,stroke:#C62828,color:#111;
    classDef warn fill:#FFF3E0,stroke:#EF6C00,color:#111;
    classDef good fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef system fill:#E3F2FD,stroke:#1565C0,color:#111;
...
```mermaid id="penurunan-jobdesc-rbm-mobile"
flowchart TD
    A["Asset<br/>Criticality"] --> B["Risk<br/>Exposure"]
    B --> C["Decision<br/>Role"]
    C --> D["Authority<br/>Level"]
    D --> E["Escalation<br/>Obligation"]
    E --> F["Job Description<br/>Berbasis Risiko"]
    ...
    classDef jobdesc fill:#F3E5F5,stroke:#6A1B9A,color:#111;
...
```mermaid id="risk-based-role-ladder-mobile"
flowchart TD
    L7["L7 Manager<br/>Risk Acceptance"]
    L6["L6 Superintendent<br/>Operational Risk Owner"]
    L5["L5 Senior Engineer<br/>Technical Validation"]
    L4["L4 Engineer<br/>Technical Decision"]
    L3["L3 Supervisor<br/>Decision Router"]
    L2["L2 Foreman<br/>Field Control"]
    L1["L1 Teknisi<br/>Early Detector"]

    L1 --> L2 --> L3 --> L4 --> L5 --> L6 --> L7

    classDef field fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef route fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef tech fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef owner fill:#F3E5F5,stroke:#6A1B9A,color:#111;
    classDef accept fill:#FFEBEE,stroke:#C62828,color:#111;
...
```mermaid id="field-escalation-stopwork-mobile"
flowchart TD
    A["Abnormality<br/>Detected"] --> B{"Risk Trigger?"}
    B -- "No" --> C["Continue Work<br/>Within SOP"]
    B -- "Yes" --> D["Stop-Work<br/>Immediately"]
    D --> E["Foreman<br/>Validate Field"]
    E --> F{"Risk Class<br/>Changed?"}
    F -- "No" --> G["Resume with<br/>Controlled Method"]
    F -- "Yes" --> H["Supervisor<br/>Escalates"]
    H --> I["Engineer / Senior Engineer<br/>Technical Review"]
    I --> J["Decision:<br/>Repair / Defer / Fix"]

    classDef detect fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef decision fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef stop fill:#FFEBEE,stroke:#C62828,color:#111;
    classDef control fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef tech fill:#F3E5F5,stroke:#6A1B9A,color:#111;

    class A detect;
    class B,F decision;
    class D stop;
    class C,E,G,H control;
    class I,J tech;
```
...
```mermaid id="engineer-senior-engineer-decision-mobile"
flowchart TD
    A["Field Data<br/>Deviation / Failure"] --> B["Engineer<br/>Technical Assessment"]
    B --> C{"Risk<br/>Acceptable?"}
    C -- "Yes" --> D["Controlled<br/>Temporary Fix"]
    C -- "No" --> E["Immediate<br/>Repair Required"]
    D --> F{"High Risk<br/>or Critical Asset?"}
    E --> F
    F -- "No" --> G["Engineer<br/>Documents Decision"]
    F -- "Yes" --> H["Senior Engineer<br/>Validation"]
    H --> I["Temporary Risk<br/>Acceptance Limit"]
    I --> J["Monitoring +<br/>Permanent Fix Plan"]

    classDef data fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef assess fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef decision fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef risk fill:#FFEBEE,stroke:#C62828,color:#111;
    classDef doc fill:#F3E5F5,stroke:#6A1B9A,color:#111;
...
```mermaid id="risk-ownership-acceptance-mobile"
flowchart TD
    A["L1 Teknisi<br/>Detect & Stop"] --> B["L2 Foreman<br/>Field Buffer"]
    B --> C["L3 Supervisor<br/>Route Decision"]
    C --> D["L4 Engineer<br/>Technical Decision"]
    D --> E["L5 Senior Engineer<br/>Risk Validation"]
    E --> F["L6 Superintendent<br/>Operational Risk Owner"]
    F --> G["L7 Manager<br/>Final Risk Acceptance"]

    G -. "Risk Tolerance" .-> F
    F -. "Operational Direction" .-> C
    E -. "Technical Boundary" .-> D
    C -. "Escalation Rule" .-> A
...
```mermaid id="role-focus-map-mobile"
flowchart TD
    A["L1-L2<br/>Field Discipline"] --> B["Detect<br/>Validate<br/>Stop"]
    C["L3<br/>Operational Control"] --> D["Delay<br/>Route<br/>Escalate"]
    E["L4-L5<br/>Technical Authority"] --> F["Analyze<br/>Decide<br/>Validate"]
    G["L6<br/>Operational Owner"] --> H["Coordinate<br/>Protect<br/>Balance"]
    I["L7<br/>Risk Acceptance"] --> J["Approve<br/>Govern<br/>Audit"]
...
```mermaid id="jobdesc-kpi-evaluation-loop-mobile"
flowchart TD
    A["JobDesc<br/>Risk Role"] --> B["KPI<br/>Risk Behavior"]
    B --> C["Evaluation<br/>Loop"]
    C --> D["Gap:<br/>Skill / Authority"]
    D --> E["Action:<br/>Training / Reposition"]
    E --> F["Updated<br/>JobDesc"]
    F --> A

    C --> G["Accountability<br/>Audit Trail"]
    G --> B

    classDef job fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef kpi fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef eval fill:#FFF8E1,stroke:#F9A825,color:#111;
    classDef gap fill:#FFEBEE,stroke:#C62828,color:#111;
...
```mermaid id="anti-overlap-clause-mobile"
flowchart TD
    A["Artikel Ini<br/>Fokus"] --> B["JobDesc<br/>Berbasis Risiko"]
    B --> C["Peran"]
    B --> D["Kewenangan"]
    B --> E["Eskalasi"]
    B --> F["Risk Ownership"]
    ...
    A --> G["Tidak Dibahas"]
    G --> H["Manpower<br/>Planning"]
    G --> I["Desain<br/>Organisasi"]
    G --> J["Training<br/>Detail"]
    G --> K["Remunerasi"]

    classDef focus fill:#E8F5E9,stroke:#2E7D32,color:#111;
    classDef scope fill:#E3F2FD,stroke:#1565C0,color:#111;
    classDef exclude fill:#FFEBEE,stroke:#C62828,color:#111;

    class A,B,C,D,E,F focus;
    class G scope;
    class H,I,J,K exclude;
```
